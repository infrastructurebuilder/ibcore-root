...
abstract public class AbstractBaseRelativeRoot implements RelativeRoot {
  ...
  public final static Optional<URI> fromString(String url) {
    URI _uri = null;
    try {
      _uri = requireNonNull(URI.create(url));
  ...
  protected transient final URI uri;
  ...
  protected AbstractBaseRelativeRoot(String u) {
    ...
    this.uri = fromString(requireNonNull(u)).orElse(null);
    this.url = fromURL(this.uri).orElse(null);
    ...
    this.stringRoot = (!u.endsWith(aref.get())) ? u + aref.get() : u;
  ...
  public final Optional<String> resolvePath(Path p) {
    ...
    return of(String.format("%s%s", this.stringRoot, p.toString()));
  }
