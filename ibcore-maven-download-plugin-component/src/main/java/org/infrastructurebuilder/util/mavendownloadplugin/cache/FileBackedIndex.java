...
/**
 * Binary file backed index. The implementation is <b>NOT</b> thread safe and should be synchronized by the lock.
 *
...
final class FileBackedIndex implements FileIndex {
  ...
  private final PathRefPath storage;
...
//    FileBackedIndex(final File baseDir, Log log) {
  FileBackedIndex(final PathRefFileSystem baseDir, Logger log) {
    this.log = log;
    this.storage = baseDir.getPath(CACHE_FILENAME);
  }
  ...
  public String get(final URI uri) {
    try {
      ...
      log.warn("Error while reading from cache " + storage);
    }
  ...
  private void load(final Path store) throws IncompatibleIndexException, IOException {
    if (Files.exists(store) && Files.size(store) != 0L) {
      try (final FileChannel channel = FileChannel.open(store, StandardOpenOption.READ);
   ...
   */
  private void save() {
    try (final OutputStream file = Files.newOutputStream(this.storage);
        final ObjectOutput res = new ObjectOutputStream(file);
        final FileChannel channel = FileChannel.open(storage, StandardOpenOption.WRITE);
