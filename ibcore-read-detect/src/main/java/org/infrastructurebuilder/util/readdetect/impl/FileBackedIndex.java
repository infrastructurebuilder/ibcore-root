...
/**
 * Persistent cache implementation of the {@link HttpCacheStorage} interface, to be used with the Apache HttpClient
 * Cache, using a directory with the copy of the files and a serialized file map.
 *
...
public final class FileBackedIndex implements HttpCacheStorage {
  ...
  private static HttpCacheEntry asHttpCacheEntry(Path path, Path cacheDir) {
    ...
    return new HttpCacheEntry(lastModifiedDate, Date.from(Instant.now()), OK_STATUS_LINE, new Header[] {
        new BasicHeader(HttpHeaders.DATE, DateUtils.formatDate(lastModifiedDate)),
        new BasicHeader(HeaderConstants.CACHE_CONTROL_MAX_AGE, String.valueOf(Integer.MAX_VALUE)),
        new BasicHeader(HeaderConstants.EXPIRES, DateUtils.formatDate(Date.from(Instant.now().plus(365, DAYS))))
  ...
  /**
   * Creates index backed by file "index.ser" in baseDir. Throws runtime exceptions if baseDir does not exist, not a
   * directory or file can't be created in it.
   *
   * @param baseDir directory where the index file should be stored.
   */
  public FileBackedIndex(final Path baseDir, Logger log) throws NotDirectoryException {
    this.log = log;
  ...
  public HttpCacheEntry getEntry(String uriString) {
    ...
    if (!this.index.containsKey(uri)) {
      log.debug(
          "Current cache: " + this.index.keySet().stream().map(u -> "\"" + u + "\"").collect(Collectors.joining(", "))
              + " does not contain \"" + uri + "\"");
      return null;
    }
   ...
   */
  private void save() {
    try (final FileOutputStream file = new FileOutputStream(this.cacheIndexFile.toFile());
