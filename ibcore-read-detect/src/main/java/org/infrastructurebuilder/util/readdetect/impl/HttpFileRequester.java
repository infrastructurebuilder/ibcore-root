...
//        }
    ...
    public HttpFileRequester build() throws /* MojoExecutionException */ IBResourceException {
      ...
      /*
       * if (isNotBlank(this.serverId)) { requireNonNull(this.mavenSession);
       *
       * if (this.log.isDebugEnabled()) { this.log.debug("providing custom authentication for " + this.serverId); }
       * final Server server = this.mavenSession.getSettings().getServer(serverId); if (server == null) { throw new
       * MojoExecutionException(String.format("Server %s not found", serverId)); } if (this.log.isDebugEnabled()) {
       * this.log.debug(String.format("serverId %s supplies username: %s and password: ***", serverId,
       * server.getUsername())); } instance.credentialsProvider.setCredentials( new AuthScope(this.uri.getHost(),
       * this.uri.getPort()), new UsernamePasswordCredentials(server.getUsername(), server.getPassword())); } else
       */ if (!WGet.isBlank(this.username)) {
        if (this.log.isDebugEnabled()) {
          this.log.debug("providing custom authentication");
          this.log.debug("username: " + username + " and password: ***");
        }
        instance.credentialsProvider.setCredentials(new AuthScope(this.uri.getHost(), this.uri.getPort()),
            new UsernamePasswordCredentials(this.username, this.password));
      }

      if (!WGet.isBlank(this.proxyHost)) {
        ...
        if (!WGet.isBlank(this.proxyUserName) && !WGet.isBlank(this.proxyPassword)) {
          instance.credentialsProvider.setCredentials(new AuthScope(host.getHostName(), host.getPort()),
              !WGet.isBlank(this.proxyNtlmHost) && !WGet.isBlank(this.proxyNtlmDomain)
                  ? new NTCredentials(this.proxyUserName, this.proxyPassword, this.proxyNtlmHost, this.proxyNtlmDomain)
  ...
  private Object handleResponse(URI uri, File outputFile, HttpCacheContext clientContext, HttpResponse response)
    ...
    if (entity != null) {
      ...
      default:
        ...
        try (InputStream in = entity.getContent(); OutputStream out = Files.newOutputStream(outputFile.toPath())) {
          int bytesRead;
  ...
  private CachingHttpClientBuilder createHttpClientBuilder() throws NotDirectoryException {
    final RequestConfig requestConfig = RequestConfig.custom().setConnectTimeout(connectTimeout)
        .setSocketTimeout(socketTimeout).setRedirectsEnabled(redirectsEnabled).build();
    final CachingHttpClientBuilder httpClientBuilder = (CachingHttpClientBuilder) CachingHttpClients.custom()
        .setDefaultCredentialsProvider(this.credentialsProvider).setRoutePlanner(routePlanner)
        .setDefaultRequestConfig(requestConfig);
    if (cacheDir != null) {
      CacheConfig config = CacheConfig.custom().setHeuristicDefaultLifetime(HEURISTIC_DEFAULT_LIFETIME)
          .setHeuristicCachingEnabled(true).setMaxObjectSize(Long.MAX_VALUE).setMaxCacheEntries(Integer.MAX_VALUE)
          .build();
      httpClientBuilder.setCacheDir(this.cacheDir).setCacheConfig(config)
          .setResourceFactory(new FileIndexResourceFactory(this.cacheDir.toPath()))
          .setHttpCacheStorage(new FileBackedIndex(this.cacheDir.toPath(), this.log)).setDeleteCache(false);
    }
