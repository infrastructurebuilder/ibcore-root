...
public class GenericRecordMapProxy implements Map<String, Object>, Supplier<GenericRecord> {
  ...
  public GenericRecordMapProxy(GenericRecord r) {
    ...
    this.fieldNames = this.r.getSchema().getFields().stream().map(Field::name).toList();
  }
  ...
  public Collection<Object> values() {
    return fieldNames.stream().map(r::get).toList();
  }

  @Override
  public Set<Entry<String, Object>> entrySet() {
    // FIXME Incorrect AND inefficient. Make local Entry. This doesn't conform to actual Entry specs, because it doesn't
    // reference backing "map"
